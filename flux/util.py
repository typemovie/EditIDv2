...
configs = {
    "flux-dev": ModelSpec(
        ...
        # ckpt_path="/home/gdli7/ComfyUI/models/checkpoints/flux-dev-fp8-12g.safetensors", # flux-dev-fp8.safetensors
        ckpt_path= "/dev_share/gdli7/models/unet/flux1-dev.sft",
        # "/share/AI-ModelScope/flux1-dev.safetensors"
        # '/home/gdli7/ComfyUI/models/checkpoints/flux-dev-fp8-12g.safetensors'
        params=FluxParams(
            in_channels=64,
            vec_in_dim=768,
            context_in_dim=4096,
        ...
        ae_path='/share/AI-ModelScope/ae.safetensors',
        ae_params=AutoEncoderParams(
            resolution=256,
            in_channels=3,
            ch=128,
...
def load_flow_model_quintized(name: str, device: str = "cuda", hf_download: bool = True):
    # Loading Flux
    print("Init model")
    ckpt_path = '/share/gdli7/models/checkpoints/flux-dev-fp8-12g.safetensors'
    if (
        not os.path.exists(ckpt_path)
        and hf_download
    ):
    ...
    json_path = "/share/gdli7/models/checkpoints/flux_dev_quantization_map.json"
...
def load_t5(device: str = "cuda", max_length: int = 512) -> HFEmbedder:
    ...
    return HFEmbedder("/share/AI-ModelScope/xflux_text_encoders", max_length=max_length, torch_dtype=torch.bfloat16).to(device)


def load_clip(device: str = "cuda") -> HFEmbedder:
    return HFEmbedder("/dev_share/gdli7/models/clip/clip-vit-large-patch14", max_length=77, torch_dtype=torch.bfloat16).to(device)
